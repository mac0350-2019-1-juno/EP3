...
def sigin(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # process the data in form.cleaned_data as required
            with connections['juno_access'].cursor() as cursor:
                cursor.execute("SELECT * FROM create_usuario(%s, %s)", [form.cleaned_data['email'], form.cleaned_data['senha']])
                result = cursor.fetchone()

                if result == None:
                    content = "Ocorreu um erro ao tentar criar o usu&aacute;rio."
                else:
                    content = "Usu&aacute;rio criado com sucesso."
            # redirect to a new URL:
            return render(request, "acesso.html", {'content':content})
